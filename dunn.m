...
xmax = 120; % 120
  ...
  kappa = kapamp*exp(-((x+70)/10).^2)+...
      kapamp*exp(-((x-70)/10).^2); %Damping
  ...
  n_it = 1:300; % number of iterations
  ...
  for j=1:length(n_it)
      ...
      [on , pos_onetwo(1)] = min(abs(tdata-0.6));
      [tw , pos_onetwo(2)] = min(abs(tdata-1.5)); 
