...
xmax = 95; % 120
  ...
  kappa = kapamp*exp(-((x+40)/5).^2)+...
      kapamp*exp(-((x-40)/5).^2); %Damping
  ...
  n_it = 1:200; % number of iterations
  ...
  for j=1:length(n_it)
      ...
      [on , pos_onetwo(1)] = min(abs(tdata-0.4));
      [tw , pos_onetwo(2)] = min(abs(tdata-0.8)); 
      ...
      v=fft(udata1);          waitbar(j/(length(n_it)))
  end
